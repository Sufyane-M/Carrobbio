...
export const rateLimiter = (config = {}) => {
    ...
    return async (req, res, next) => {
        try {
            ...
            const blockedAttempts = recentAttempts?.filter(attempt => attempt.is_blocked && new Date(attempt.attempted_at) > blockedUntil) || [];
 ...
 */
export const logLoginAttempt = async (req, isSuccessful, userId, errorMessage) => {
    try {
        const clientIp = req.ip || req.connection.remoteAddress || 'unknown';
        const userAgent = req.get('User-Agent') || 'unknown';
        const endpoint = req.path;
