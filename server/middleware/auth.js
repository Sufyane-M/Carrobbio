 ...
 */
async function logSecurityEvent(adminId, action, ipAddress, userAgent, success = true, errorMessage) {
    try {
        await supabase
            .from('audit_logs')
            .insert({
...
export const requireRole = (allowedRoles) => {
    return async (req, res, next) => {
        ...
        if (!allowedRoles.includes(req.adminUser.role)) {
            await logSecurityEvent(req.adminUser.id, 'AUTH_INSUFFICIENT_PERMISSIONS', ipAddress, userAgent, false, `Required roles: ${allowedRoles.join(', ')}, User role: ${req.adminUser.role}`);
            res.status(403).json({
                success: false,
 ...
 */
export const validateInput = (schema, target = 'body') => {
    return (req, res, next) => {
        try {
 ...
 */
export const rateLimitByIP = (maxRequests = 100, windowMinutes = 15) => {
    return async (req, res, next) => {
        const ipAddress = req.ip || req.connection.remoteAddress || 'unknown';
        const userAgent = req.get('User-Agent');
 ...
 */
export const logSensitiveOperation = (operationType) => {
    return async (req, res, next) => {
        const ipAddress = req.ip || req.connection.remoteAddress || 'unknown';
        const userAgent = req.get('User-Agent');
        const adminId = req.adminId;
        try {
            await logSecurityEvent(adminId || null, `SENSITIVE_OP_${operationType.toUpperCase()}`, ipAddress, userAgent, true, `${req.method} ${req.path} - ${JSON.stringify(req.body)}`);
            next();
        }
        catch (error) {
