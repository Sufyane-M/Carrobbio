...
const changePasswordSchema = z.object({
    ...
    newPassword: z.string()
        ...
        .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#+\-_=])/, 'Password deve contenere almeno: 1 minuscola, 1 maiuscola, 1 numero, 1 carattere speciale')
});
 ...
 */
function generateJWT(adminId, email, role) {
    const payload = {
        adminId,
        email,
        role,
...
export const refreshToken = async (req, res) => {
    ...
    try {
        ...
        const newAccessToken = generateJWT(session.admin_users.id, session.admin_users.email, session.admin_users.role);
        // Aggiorna sessione
        await supabase
            .from('admin_sessions')
            .update({
...
export const changePassword = async (req, res) => {
    const ipAddress = req.ip || req.connection.remoteAddress || 'unknown';
    const userAgent = req.get('User-Agent');
    const adminId = req.adminId; // Impostato dal middleware di autenticazione
    try {
        // Validazione input
