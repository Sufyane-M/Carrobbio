...
import { authenticateToken, requireRole, requirePasswordChange, validateInput, securityHeaders, logSensitiveOperation } from '../middleware/auth.js';
import { rateLimiter, loginRateLimiter, passwordResetRateLimiter } from '../middleware/rateLimiter.js';
import { z } from 'zod';
/**
...
const ChangePasswordSchema = z.object({
    ...
    newPassword: z.string()
        ...
        .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/, 'La password deve contenere almeno: 1 minuscola, 1 maiuscola, 1 numero, 1 carattere speciale')
});
 ...
 */
router.post('/login', loginRateLimiter, validateInput(LoginSchema), logSensitiveOperation('LOGIN_ATTEMPT'), authController.login);
/**
 ...
 */
router.post('/logout', authenticateToken, logSensitiveOperation('LOGOUT_ATTEMPT'), authController.logout);
/**
 ...
 */
router.post('/change-password', authenticateToken, validateInput(ChangePasswordSchema), logSensitiveOperation('PASSWORD_CHANGE_ATTEMPT'), authController.changePassword);
/**
 ...
 */
router.post('/force-password-change', authenticateToken, requirePasswordChange, validateInput(ChangePasswordSchema), logSensitiveOperation('FORCED_PASSWORD_CHANGE'), authController.changePassword);
/**
 ...
 */
router.post('/request-password-reset', passwordResetRateLimiter, validateInput(ResetPasswordSchema), logSensitiveOperation('PASSWORD_RESET_REQUEST'), authController.requestPasswordReset);
/**
 ...
 * @security Token JWT richiesto, logging operazione
 */
router.delete('/sessions/:sessionId', authenticateToken, logSensitiveOperation('SESSION_TERMINATION'), authController.terminateSession);
/**
 ...
 * @security Token JWT richiesto, logging operazione
 */
router.delete('/sessions', authenticateToken, logSensitiveOperation('ALL_SESSIONS_TERMINATION'), authController.terminateAllSessions);
/**
 ...
 */
router.get('/security-stats', authenticateToken, requireRole(['admin', 'super_admin']), authController.getSecurityStats);
/**
 ...
 */
router.get('/admins', authenticateToken, requireRole(['admin', 'super_admin']), logSensitiveOperation('ADMIN_LIST_ACCESS'), authController.getAllAdmins);
/**
 ...
 */
router.post('/unlock-account', authenticateToken, requireRole(['super_admin']), validateInput(z.object({
    adminId: z.string().uuid('ID admin non valido')
})), logSensitiveOperation('ACCOUNT_UNLOCK'), authController.unlockAccount);
/**
 ...
 */
router.post('/refresh-token', rateLimiter(), validateInput(z.object({
