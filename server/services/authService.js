...
export class AuthService {
     ...
     */
    static async logout(sessionId, ipAddress, userAgent) {
        try {
     ...
     */
    static async changePassword(adminId, currentPassword, newPassword, ipAddress, userAgent) {
        try {
            // Validazione input
    ...
    static async generateTokens(user) {
        ...
        const refreshToken = jwt.sign({ adminId: user.id, type: 'refresh' }, JWT_REFRESH_SECRET, {
     ...
     */
    static async createSession(adminId, sessionToken, ipAddress, userAgent) {
     ...
     */
    static async handleFailedLogin(adminId, ipAddress, userAgent) {
        // Incrementa tentativi falliti
     ...
     */
    static async resetFailedLoginAttempts(adminId) {
        await supabase
            .from('admin_users')
            .update({
     ...
     * Registra tentativo di login
     */
    static async logLoginAttempt(email, ipAddress, userAgent, success) {
        try {
            await supabase
                .from('login_attempts')
                .insert({
     ...
     */
    static async logSecurityEvent(adminId, action, ipAddress, userAgent, success = true, details) {
        try {
            await supabase
                .from('audit_logs')
                .insert({
