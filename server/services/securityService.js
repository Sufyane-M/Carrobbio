 ...
 */
export const analyzeIPAddress = async (ipAddress, timeWindow = 60 * 60 * 1000 // 1 ora
) => {
    try {
        ...
        const suspiciousEvents = events.filter(e => e.event_type.includes('suspicious') ||
 ...
 */
export const detectAttackPatterns = async (timeWindow = 60 * 60 * 1000 // 1 ora
) => {
    try {
        const windowStart = new Date(Date.now() - timeWindow);
        ...
        const bruteForceEvents = events.filter(e => e.event_type.includes('login_failure') ||
        ...
        const sqlInjectionEvents = events.filter(e => e.details && JSON.stringify(e.details).includes('sql'));
        ...
        const xssEvents = events.filter(e => e.details && JSON.stringify(e.details).includes('script'));
        ...
        const blockedIPs = Object.entries(ipCounts)
            .filter(([_, count]) => count > SECURITY_CONFIG.SUSPICIOUS_ACTIVITY_THRESHOLD)
            .map(([ip, _]) => ip);
        ...
        const maxHourlyEvents = Math.max(...Object.values(hourlyDistribution).map(v => v));
 ...
 */
export const getSecurityStatistics = async (timeWindow = 24 * 60 * 60 * 1000 // 24 ore
) => {
    try {
        const windowStart = new Date(Date.now() - timeWindow);
        ...
        const blockedIPs = Object.values(ipCounts).filter((count) => count >= SECURITY_CONFIG.MAX_LOGIN_ATTEMPTS).length;
        // Utenti attivi unici
 ...
 */
export const getLoginAttempts = async (userId, timeWindow = 24 * 60 * 60 * 1000 // 24 ore
) => {
    try {
        const windowStart = new Date(Date.now() - timeWindow);
 ...
 */
export const blockSuspiciousIP = async (ipAddress, reason, duration = SECURITY_CONFIG.IP_BLOCK_DURATION) => {
    try {
...
export const monitorSuspiciousActivity = async () => {
    try {
        ...
        for (const [ip, events] of Object.entries(ipActivity)) {
            ...
            const suspiciousEvents = eventArray.filter(e => e.severity === 'high' ||
                e.event_type.includes('attack') ||
                e.event_type.includes('suspicious'));
            if (suspiciousEvents.length >= 3) {
                await blockSuspiciousIP(ip, `${suspiciousEvents.length} eventi sospetti in 5 minuti`);
                ...
                if (adminUsers && adminUsers.length > 0) {
                    for (const admin of adminUsers) {
                        await sendSuspiciousLoginNotification(admin.email, ip, eventArray[0].user_agent || 'unknown', new Date().toLocaleString('it-IT'));
                    }
                }
            }
        }
